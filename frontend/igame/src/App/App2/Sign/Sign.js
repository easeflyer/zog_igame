...
import {coachList,NewTeam, SignEvent } from '../Model/Deal'
...
// 请求通讯录列表  ★
    componentWillMount(){
        // 每次打开报名页都重新请求
        // const Users = new coachList(res => this.stateFriends(res));
        // Users.users();
    }
    stateFriends=(res)=>{
        if(res && res.length!==0){
            this.setState({
                originList:res,
                coachList:res.filter(item => {return item.name !== session.get_name()}),
                playerList:res.filter(item => {return item.name !== session.get_name()}),
                mySelf: res.filter( item => {return item.name === session.get_name()})[0]
            });
        }else{
            Toast.info('没有用户信息，请先添加好友！！', 2);
        }
        console.log(this.state.originList)
    }
...
// 教练 ，值发生改变 ★
    handleCoach = (key)=>{
        this.setState({
            playerList:  this.state.originList.filter(item=> { return item.name !== session.get_name() && item.id !== key;})
        })
    }
...
// 队员 ，值发生改变 ★
    handlePlayer = (key)=>{
        this.setState({
            coachList:   this.state.coachList.filter((item,index) => { return item.name !== session.get_name() && item.id !== key;})
        })
    }
    handlePlayerDe = (key)=>{
        this.setState({
            coachList:   this.state.coachList.concat(
                            this.state.playerList.filter((item,index) => { return item.name !== session.get_name() && item.id === key;})
                        )
        })
    }
            ...
            <Form layout="vertical"   onSubmit={this.onSubmit}>
                ...
                <FormItem label="教练"  style={{marginBottom:0}}>  
                    {getFieldDecorator('coach', {
                        ...
                        {this.items(this.state.coachList)}
                        </Select>   
                    )} 
                </FormItem>
                <FormItem label="队员"  style={{marginBottom:0}}>   
                    {getFieldDecorator('player', {
                        ...
                        {this.items(this.state.playerList)}
                        </Select>  
                    )}
                </FormItem>     
                <FormItem>
