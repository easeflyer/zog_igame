...
export default class PlayerInfo extends React.Component{
    ...
    sucPolling=(data)=>{
        ...
        if (data.length){
            ...
            if(body.dummy&&body.openlead&&body.declarer){   //收到叫牌结果信息   {dummy:'N',openlead:'W',declarer:'S',nextplayer:'W',contract:'1S'}
                ...
                this.props.setTopInfo2(body.declarer,body.contract)
            }
            if(body.number&&body.rank&&body.card){   //收到打牌消息 {declarer_win:0,number:1,rank:'5',pos:'W',suit:'C',nextplayer:'W',card:'C5',opp_win:0}
                let res = DealFunc.play_card(body,this.state.myCardsNum,this.state.dummyCardsNum,this.state.playerInfo,this.state.playCards)
                console.log(res)
                this.setState({
                    currentDirect:body.nextplayer,
                })
                // if(body.number===52){
                //     this.post('init_board',this.state.id_msg.board_id+1,this.state.id_msg.channel_id)
                //     this.setState({
                //         callCards:new Initial().callCards,
                //         playCards:new Initial().playCards,
                //         currentDirect:null,
    ...
    post=(method,...data)=>{
        ...
        (this.state.currentDirect===this.state.dummy&&this.state.declarer===this.state.playerInfo.myDirect)) ? board.play(this.state.id_msg.board_id,this.this.state.playerInfo.myDirect,...data,this.state.id_msg.channel_id) : null;       //发送打牌消息
    } 
    sucPost=(data)=>{  
        ...
        return(
            <div>
                ...
                <Row style={{marginBottom:10}}>
                    ...
                    <Col span={20} style={{background:'#ff0',paddingLeft:10}}>{this.state.playerInfo.topName}{this.state.call&&this.state.callDirect===this.state.playerInfo.topDirect?'该你叫牌啦！':null}{this.state.play&&this.state.declarer===this.state.playerInfo.topDirect?' 庄家':null}{this.state.currentDirect===this.state.playerInfo.topDirect?'★':null}</Col>
                ...
                {/* 左 */}
                    <Col span={2}>
                        <Row style={{height:300,writingMode: 'vertical-lr'}}>
                            ...
                            <Col span={24} style={{height:240,background:'#ff0'}}>{this.state.playerInfo.leftName}{this.state.call&&this.state.callDirect===this.state.playerInfo.leftDirect?'该你叫牌啦！':null}{this.state.play&&this.state.declarer===this.state.playerInfo.leftDirect?' 庄家':null}{this.state.currentDirect===this.state.playerInfo.leftDirect?'★':null}</Col>
                        </Row>
                    </Col>
                ...
                <Row style={{marginTop:10}}>
                    ...
                    <Col span={20} style={{background:'#ff0',paddingLeft:10}}>{this.state.playerInfo.myName}{this.state.call&&this.state.callDirect===this.state.playerInfo.myDirect?'该你叫牌啦！':null}{this.state.play&&this.state.declarer===this.state.playerInfo.myDirect?' 庄家':null}{this.state.currentDirect===this.state.playerInfo.myDirect?'★':null}</Col>
                </Row>
            </div>
        )
    }
